...
function ProductState(arrays::AbstractArray{<:AbstractVector})
    ...
    for coord in eachindex(IndexCartesian(), arrays)
        _tensor = NamedTensor(arrays[coord], [Index(plug"$coord")])
        addtensor!(tn, _tensor)
        setsite!(tn, _tensor, site"$coord")
        setlink!(tn, Index(plug"$coord"), plug"$coord")
    end
...
function ProductOperator(arrays::AbstractArray{<:AbstractMatrix})
    ...
    for coord in eachindex(IndexCartesian(), arrays)
        _tensor = NamedTensor(arrays[coord], [Index(plug"$coord"), Index(plug"$coord'")])
        addtensor!(tn, _tensor)
        setsite!(tn, _tensor, site"$coord")
        setlink!(tn, Index(plug"$coord"), plug"$coord")
        setlink!(tn, Index(plug"$coord'"), plug"$coord'")
...
function ProductOperator(tensors::AbstractArray{<:Tensor})
    ...
    for coord in eachindex(IndexCartesian(), tensors)
        ...
        setlink!(tn, ind_out, plug"$coord")
        setlink!(tn, ind_in, plug"$coord'")
    end
...
function Base.rand(rng::Random.AbstractRNG, ::Type{ProductState}; n, eltype=Float64, physdim=2)
    ...
    for i in 1:n
        ...
        tn[site"$i"] = NamedTensor(site_vec, [Index(plug"$i")])
