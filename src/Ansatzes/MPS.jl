...
function MPS(arrays::AbstractVector{<:AbstractArray}; order=defaultorder(MPS)) # , check=true)
    ...
    for (i, array) in enumerate(arrays)
        ...
        haslink(tn, bond"$i-$isup") ||
            hasind(tn, Index(bond"$i-$isup")) && setlink!(tn, Index(bond"$i-$isup"), bond"$i-$isup")
        haslink(tn, bond"$isub-$i") ||
            hasind(tn, Index(bond"$isub-$i")) && setlink!(tn, Index(bond"$isub-$i"), bond"$isub-$i")
    end
...
function Base.convert(::Type{MPS}, old_tn::ProductState)
    ...
    for i in 1:n
        ...
        setlink!(tn, Index(plug"$i"), plug"$i")
        if i != 1 && !haslink(tn, bond"$(i - 1)-$i")
            setlink!(tn, Index(bond"$(i - 1)-$i"), bond"$(i - 1)-$i")
        end
        if i != n && !haslink(tn, bond"$i-$(i + 1)")
            setlink!(tn, Index(bond"$i-$(i + 1)"), bond"$i-$(i + 1)")
        end
    end
