...
function generic_mps_compress!(tn, _bond; maxdim=nothing, threshold=nothing, kwargs...)
    @assert !isnothing(maxdim) || !isnothing(threshold) "Either `maxdim` or `threshold` must be specified"
    @assert isnothing(maxdim) || maxdim > 0 "maxdim must be a positive integer"
    @assert isnothing(threshold) || threshold > 0 "Threshold must be positive"
