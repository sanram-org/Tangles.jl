...
function generic_evolve_mps_mpo_direct!(mps, op)
    @assert nsites(mps) == nsites(op) "MPS and MPO must have the same number of sites"
    ...
    @unsafe_region mps for i in 1:nsites(mps)
        ...
        if i > 1
            j = i - 1
            c = fuse(c, [ind_at(mps, bond"$j - $i"), ind_at(op, bond"$j - $i")], ind_at(mps, bond"$j - $i"))
        end
        if i < nsites(mps)
            j = i + 1
            c = fuse(c, [ind_at(mps, bond"$i - $j"), ind_at(op, bond"$i - $j")], ind_at(mps, bond"$i - $j"))
        end
...
function generic_evolve_mps_mpo_zipup!(mps, op; maxdim=nothing, threhold=nothing)
    @assert nsites(mps) == nsites(op) "MPS and MPO must have the same number of sites"
    @assert !isnothing(maxdim) || !isnothing(threhold) "Either `maxdim` or `threhold` must be specified, or use"
