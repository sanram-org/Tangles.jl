using Test
using Tangles
using Tangles: array, arrays
using Tangles: checkform, orthog_center, min_orthog_center, max_orthog_center, unsafe_setform!, LambdaSite
...
@testset "constructor: case 2: order = [:r, :o, :l]" begin
    ...
    @test array(tn[site"1"]) == a
    @test array(tn[site"2"]) == b
    @test array(tn[site"3"]) == c
...
@testset "conversion: ProductState -> MPS" begin
    ...
    @test vec(array(b[site"1"])) == [1.0, 0.0]
    @test vec(array(b[site"2"])) == [0.0, 1.0]
    @test vec(array(b[site"3"])) == [1.0, 1.0] / sqrt(2)
end
